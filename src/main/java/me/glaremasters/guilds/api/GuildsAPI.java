...
public class GuildsAPI {
    ...
    /*public static Leaderboard createLeaderboard(String name, Leaderboard.LeaderboardType leaderboardType, Leaderboard.SortType sortType) {
        Leaderboard leaderboard = Main.getInstance().getLeaderboardHandler().getLeaderboards().stream().filter(l -> l.getName().equals(name) && l
        .getLeaderboardType() == leaderboardType).findFirst().orElse(null);
