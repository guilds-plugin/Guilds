...
public class CommandAlly extends CommandBase {
    ...
    public CommandAlly() {
        super("ally", "Ally commands", "guilds.command.ally", false, null,
    ...
    @Override public void execute(Player player, String[] args) {
        ...
        if (args[0].equals("accept")) {
            ...
            guild.getMembers().forEach(member -> Message
                .sendMessage(Bukkit.getPlayer(member.getUniqueId()),
                    Message.COMMAND_ALLY_ACCEPTED.replace("{guild}", targetGuild.getName())));
            targetGuild.getMembers().forEach(member -> Message
                .sendMessage(Bukkit.getPlayer(member.getUniqueId()),
                    Message.COMMAND_ALLY_ACCEPTED_TARGET.replace("{guild}", guild.getName())));
            ...
            guild.getMembers().stream()
                ...
                member -> Message.sendMessage(Bukkit.getPlayer(member.getUniqueId()),
                    Message.COMMAND_ALLY_DECLINED.replace("{guild}", targetGuild.getName())));
        ...
        if (args[0].equalsIgnoreCase("add")) {
            ...
            if (guild.getAllies().contains(targetGuild.getName()) || guild.getName()
                ...
                Message.sendMessage(player,
                    Message.COMMAND_ALLY_ALREADY_ALLIES.replace("{guild}", targetGuild.getName()));
                return;
            }
