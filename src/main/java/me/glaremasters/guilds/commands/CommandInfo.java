...
public class CommandInfo extends CommandBase implements Listener {
  ...
  public void execute(Player player, String[] args) {
    if (Main.getInstance().getConfig().getBoolean("gui.info")) {
      ...
      beaconlore.add(
          ChatColor.AQUA + GuildRole.getRole(guild.getMember(player.getUniqueId()).getRole())
              .getName());
      ...
      Message.sendMessage(player, Message.COMMAND_INFO_NAME
          .replace("{guild}", guild.getName(), "{prefix}", guild.getPrefix()));
      Message.sendMessage(player, Message.COMMAND_INFO_MASTER.replace("{master}",
          Bukkit.getOfflinePlayer(guild.getGuildMaster().getUniqueId()).getName()));
      Message.sendMessage(player, Message.COMMAND_INFO_MEMBER_COUNT
      ...
      Message.sendMessage(player, Message.COMMAND_INFO_RANK.replace("{rank}",
          GuildRole.getRole(guild.getMember(player.getUniqueId()).getRole()).getName()));
      Message.sendMessage(player, Message.COMMAND_INFO_PLAYERS.replace("{players}",
