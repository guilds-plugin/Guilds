...
class Query {
	...
	static final String CREATE_TABLE_GUILDS =
			"CREATE TABLE IF NOT EXISTS guilds (" + "name VARCHAR(255) NOT NULL, prefix VARCHAR(255) NOT NULL, tier INT, isPrivate TINYINT(1), " +
					"PRIMARY KEY (name))";
	static final String CREATE_TABLE_ALLIES =
			"CREATE TABLE IF NOT EXISTS guild_allies(" + "name VARCHAR(255) NOT NULL, guildName VARCHAR(255) NOT NULL, " +
					"FOREIGN KEY(guildName) REFERENCES guilds(name)), PRIMARY KEY(name)";
	static final String CREATE_TABLE_MEMBERS =
			"CREATE TABLE IF NOT EXISTS guild_members ( " + "uuid VARCHAR(36) NOT NULL, guild VARCHAR(255) NOT NULL, " + "role INTEGER NOT NULL, " +
					"FOREIGN KEY(guild) REFERENCES guilds(name)), PRIMARY KEY (uuid), ";
	static final String CREATE_TABLE_INVITED_MEMBERS =
			"CREATE TABLE IF NOT EXISTS invited_members ( " + "uuid VARCHAR(36) NOT NULL, guild VARCHAR(255) NOT NULL," +
					"FOREIGN KEY (guild) REFERENCES guilds(name), PRIMARY KEY (uuid))";
	...
	static final String SELECT_ALL_GUILD_DATA = "SELECT guilds.name, guilds.prefix, guild_members.uuid, guild_members.role FROM guilds "
	...
	static final String ADD_MEMBER = "INSERT INTO guild_members (uuid, guild, role) VALUES(?, ?, ?) "
			+ "ON DUPLICATE KEY UPDATE guild=VALUES(guild), role=VALUES(role)";
	...
	static final String ADD_ALLY = "INSERT INTO guild_allies(name, guildName) VALUES(?, ?)";
	static final String REMOVE_ALLY = "DELETE FROM guild_allies WHERE name=? AND guildName=?";
	static final String FIND_ALLY = "SELECT name FROM guild_allies WHERE guildName=?";
