...
public final class Json implements DatabaseProvider {
	...
	public void getGuilds(Callback<Map<String, Guild>, Exception> callback) {
		Main.newChain()
				.asyncFirst(() -> {
					try (JsonReader reader = new JsonReader(new FileReader(guildsFile))) {
						return (Map<String, Guild>) gson.fromJson(reader, guildsType);
					} catch (IOException e) {
						e.printStackTrace();
						return null;
					}
	...
	public void getLeaderboards(Callback<List<Leaderboard>, Exception> callback) {
		Main.newChain()
				.asyncFirst(() -> {
					try (JsonReader reader = new JsonReader(new FileReader(leaderboardsFile))) {
						return (List<Leaderboard>) gson.fromJson(reader, leaderboardsType);
					} catch (IOException e) {
						e.printStackTrace();
						return null;
					}
				})
				.syncLast(leaderboards -> callback.call(leaderboards, null))
				.execute();
	}
	...
	public void updateLeaderboard(Leaderboard leaderboard, Callback<Boolean, Exception> callback) {
		List<Leaderboard> leaderboards = getLeaderboards();
		leaderboards.remove(leaderboards.stream()
				.filter(l -> l.getName().equals(leaderboard.getName()))
				.filter(l -> l.getLeaderboardType() == leaderboard.getLeaderboardType())
